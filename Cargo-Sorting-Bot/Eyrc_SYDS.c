 ...
 * Logic: Firstly it commnads the robot to traverse the indiactor block arena by visiting each box one by one, it stores the colour
 		  ...
 		  Pos[1] = some color or empty
 		  if color matches with corresponding indiactor color (As compared to Indicator[1])
 		  Buzzer for 500ms.

 		  If 
 		  color = false
 		  Check which value of array matches with that color.
 		  destiantion[1]: The destination terminal onto which the box should be kept.
...
int main()
{	
	char I1,I2,I3,I4; // to store the color values of indiactor blocks.
	char Pos[8]; // to store the color values of blocks at terminals.
	char destiantion[8]; //to store the correct destination of each block.
						// eg. if color of block at Terminal 4 Left is 'red', ie. it's destiantion is terminal 1.
						// so it stores the relevant destiantion so that it can assign it later while sorting.
	
	init_devices();
	lcd_set_4bit();
	lcd_init();
	servo_1(0); // initially keep the servo lifted up so that it doesn't hinders the  motion of robot.
	
	/* starting with indicaor blocks traversal, visiting each indicator block one by one */

	forward();
	velocity(180,180);
	_delay_ms(1000);
	stop(); // the very first node(all 3 white line sensors give a black value) after start position.
	...
	_delay_ms(200);
	stop();
	I3=check_color(); // first indicator block I3, color is being checked and stored in variable I3.
	...
	_delay_ms(1000);
	stop();
	I4=check_color(); // Second indicator block I4, color is being checked and stored in variable I4.
	...
	_delay_ms(200);
	stop();
	I1=check_color(); // third indicator block I1, color is being checked and stored in variable I1.
	...
	_delay_ms(200);
	stop();
	I2=check_color(); // fourth indicator block I2, color is being checked and stored in variable I.
	...
	/* indicator blocks traversal completed.
	* Traversing of main arena starts.
	*The robot will now reach the centermost mode of arena which further branches into terminals.
	*/
	blackline();

	//traversal of left half of arena.
 	turn_left();
	blackline();
	...
	stop();
	Pos[0]=check_color();
	if(Pos[0]==I1) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	}
	else    //decides the correct destination of the box, which it visits while traversing the arena.
	{
		switch (Information_zero)
		case 'A' :
			if (Pos[0]==I3)
			{
				char destination[0] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[0]==I2)
			{
				char destination[0] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[0]==I4)
			{
				char destination[0] = 'T4';
			}
			break;
			default
				char destiantion[0] = 0;
	...
	stop();
	Pos[1]=check_color();
	if(Pos[1]==I1) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_one)
		case 'A' :
			if (Pos[1]==I3)
			{
				char destination[1] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[1]==I2)
			{
				char destination[1] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[1]==I4)
			{
				char destination[1] = 'T4';
			}
			break;
			default
				char destiantion[1] = 0;

	}
	//	terminal one traversal finished.
	turn_right();
	...
	stop();
	Pos[2]=check_color();
	if(Pos[2]==I3) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_two)
		case 'A' :
			if (Pos[2]==I3)
			{
				char destination[2] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[2]==I2)
			{
				char destination[2] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[2]==I4)
			{
				char destination[2] = 'T4';
			}
			break;
			default
				char destiantion[2] = 0;
	...
	stop();
	Pos[3]=check_color();
	if(Pos[3]==I3) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_three)
		case 'A' :
			if (Pos[3]==I3)
			{
				char destination[3] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[3]==I2)
			{
				char destination[3] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[3]==I4)
			{
				char destination[3] = 'T4';
			}
			break;
			default
				char destiantion[3] = 0;

	}
// terminal 3 traversal finishes.
	turn_right();
	blackline();
	turn_right();
	blackline();
	...
	// Robot again navigates to the Center node to traverse the right half of the arena.
	...
	stop();
	Pos[4]=check_color();
	if(Pos[4]==I2) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_four)
		case 'A' :
			if (Pos[7]==I3)
			{
				char destination[4] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[7]==I2)
			{
				char destination[4] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[7]==I4)
			{
				char destination[4] = 'T4';
			}
			break;
			default
				char destiantion[4] = 0;
	...
	stop();
	Pos[5]=check_color();
	if(Pos[5]==I2) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_five)
		case 'A' :
			if (Pos[5]==I3)
			{
				char destination[5] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[5]==I2)
			{
				char destination[5] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[5]==I4)
			{
				char destination[5] = 'T4';
			}
			break;
			default
				char destiantion[5] = 0;
	}
	// terminal 3 traversal finishes.
	turn_right();
	blackline();
	...
	stop();
	Pos[6]=check_color();
	if(Pos[6]==I4) // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_six)
		case 'A' :
			if (Pos[6]==I3)
			{
				char destination[6] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[6]==I2)
			{
				char destination[6] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[6]==I4)
			{
				char destination[6] = 'T4';
			}
			break;
			default
				char destiantion[6] = 0;
	...
	stop();
	Pos[7]=check_color();
	if(Pos[7]==I4)  // when the color is correct when compared to corresponding indicator block color it gives a beep for 500 ms.
	{
		buzzer_on();
		_delay_ms(500);
		buzzer_off();
	...
	{
		switch (Information_seven)
		case 'A' :
			if (Pos[7]==I3)
			{
				char destination[7] = 'T3';
			}
			break;
		case 'B' :	
			if (Pos[7]==I2)
			{
				char destination[7] = 'T2';
			}
			break;
		case 'C' :
			if (Pos[7]==I4)
			{
				char destination[7] = 'T4';
			}
			break;
			default
				char destiantion[7] = 0;
		
	}
	//terminal 4 traversal finishes.
	/* 	Till here traversing of whole arena is done once and sensors
	*	 have collected the required information.
	*  	Now we have destiantions assigned in variable 
	*	'destination' for all the blocks it visits.
	*/
	...
	// T4-R to T1-R, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T1-R to T3-L picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T3-L to T4-L, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	turn_right();
	...
	// T4 - L to T4-R, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T4-R to T3-L, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T3-L to T3-R, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T3-R to T4-R, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T4-R to T2-R, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
	...
	// T2-R to T3-R, picks box and keeps it on assigned destiantion and updates the corresponding values in arrays "pos".
